...
const abandon = async (): Promise<void> => { const result = await props.training.abandonProgramCycle(); if (!result.ok) { error.value = "Finaliza o descarta la sesión en curso antes de cambiar de programa."; return; } await load(); };
...
<template>
  <section aria-labelledby="program-title"><header><p class="eyebrow">Plan local</p><h2 id="program-title">Programa</h2></header>
    ...
    <template v-if="progress"><p>{{ progress.cycle.programName }} · {{ progress.completedSessions }}/{{ progress.plannedSessions }} sesiones · adherencia {{ Math.round(progress.adherence * 100) }}%</p><p v-if="progress.nextSession">Semana {{ progress.nextSession.week }}, sesión {{ progress.nextSession.position + 1 }}</p><p v-else-if="progress.cycle.status === 'abandoned'">Ciclo abandonado. Puedes iniciar otro programa sin perder el historial registrado.</p><p v-else>Informe final · {{ progress.cycle.goals.filter((goal) => goal.achieved).length }}/{{ progress.cycle.goals.length }} metas alcanzadas.</p><button v-if="progress.nextSession" type="button" @click="next">Iniciar próxima sesión</button><button v-if="progress.nextSession" type="button" class="secondary-action" @click="skip">Omitir sesión</button><button v-if="progress.nextSession" type="button" class="secondary-action" @click="abandon">Cambiar programa</button><button v-if="!progress.nextSession && progress.cycle.status === 'completed'" type="button" @click="duplicate">Repetir ciclo</button><ul class="card-list"><li v-for="goal in progress.cycle.goals" :key="goal.exerciseId"><strong>{{ goal.exerciseId }}</strong><span>{{ goal.achieved ? "Meta alcanzada" : "En progreso" }}</span><span v-if="goal.recommendedWeight">Siguiente: {{ goal.recommendedWeight.amount }} {{ goal.recommendedWeight.unit }}</span></li></ul><ul v-if="!progress.nextSession" class="card-list"><li v-for="program in programs" :key="program.id"><strong>{{ program.name }}</strong><span>{{ program.weeks }} semanas · {{ program.sessions.length }} días por semana</span><button type="button" @click="start(program.id)">Iniciar programa</button></li></ul></template>
